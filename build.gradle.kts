...
tasks.register("consciousnessStatus") {
    group = "aegenesis"
    description =
        "Concise state snapshot of the substrate (toolchains, modules, cache flags). Use -Dformat=json for JSON output."
    
    // Pre-compute values during configuration phase to avoid configuration cache issues
    ...
    val useJsonFormat = (System.getProperty("format") == "json")
...
/**
 * Heuristically detects whether Jetpack Compose is enabled in the given project.
 *
 * Checks a few signals in order: presence of the Kotlin Compose Gradle plugin,
 * a `composeCompilerPlugin` configuration, and Android `buildFeatures.compose` (if an Android
 * extension is present). Returns false if none of the signals are found or if any
 * reflection/error occurs while probing.
 *
 * @param sp The project to inspect for Compose enablement.
 * @return `true` if Compose is detected, otherwise `false`.
 */
private fun Project.detectComposeEnabled(sp: Project): Boolean = runCatching {
    ...
    val buildFeatures = androidExt::class.java.methods.firstOrNull { it.name == "getBuildFeatures" }
        ?.invoke(androidExt) ?: return false
    val composeMethod =
        buildFeatures.javaClass.methods.firstOrNull { it.name == "getCompose" } ?: return false
    (composeMethod.invoke(buildFeatures) as? Boolean) == true
}.getOrElse { false }

/**
 * Safely reads the Java toolchain language version for a given project, returning it as a string.
 *
 * Uses reflective access and returns null if the Java extension, toolchain, or language version
 * cannot be determined.
 *
 * @param sp The project whose Java toolchain language version should be retrieved.
 * @return The language version string (for example "17") or `null` if unavailable or on error.
 */
private fun Project.safeJavaToolchain(sp: Project): String? = runCatching {
    val javaExt = sp.extensions.findByName("java") ?: return null
    val toolchain =
        javaExt::class.java.methods.firstOrNull { it.name == "getToolchain" }?.invoke(javaExt)
            ?: return null
    toolchain.javaClass.methods.firstOrNull { it.name == "getLanguageVersion" }?.invoke(toolchain)
        ?.toString()
}.getOrNull()

/**
 * Safely reads the Kotlin JVM target of a project's `compileKotlin` task.
 *
 * Attempts to retrieve `kotlinOptions.jvmTarget` via reflection and returns it as a string.
 * Returns `null` if the `compileKotlin` task is missing, the option is not available, or any reflection error occurs.
 *
 * @param sp The project whose `compileKotlin` task will be inspected.
 * @return The JVM target string (e.g., "1.8", "11", "17") or `null` when unavailable.
 */
private fun Project.safeKotlinJvmTarget(sp: Project): String? = runCatching {
    ...
    val opts =
        compile::class.java.methods.firstOrNull { it.name == "getKotlinOptions" }?.invoke(compile)
            ?: return null
    opts.javaClass.methods.firstOrNull { it.name == "getJvmTarget" }?.invoke(opts)?.toString()
}.getOrNull()

/**
 * Collects a ModuleReport for each subproject in this Project.
 *
 * Builds a list of ModuleReport entries by inspecting each subproject's applied plugins and
 * lightweight heuristics for toolchain/feature detection. For each subproject the report includes:
 * - name and path
 * - type: "android-app" if the Android application plugin is applied, "android-lib" for the Android
 *   library plugin, "kotlin-jvm" for the Kotlin JVM plugin, otherwise "other"
 * - javaToolchain: best-effort JVM language version string via reflective lookup (may be null)
 * - kotlinJvmTarget: best-effort Kotlin JVM target via reflective lookup (may be null)
 * - hasHilt / hasCompose / hasKsp: boolean flags derived from plugin presence and feature heuristics
 *
 * @return A list of ModuleReport, one per subproject, in the same iteration order as subprojects.
 */
...
/**
 * Convert this list of strings into a JSON array literal.
 *
 * Each element is wrapped in double quotes and elements are comma-separated.
 * Note: this is a minimal serializer and does not escape special characters inside elements.
 *
 * @return A JSON array string (e.g. `["a","b","c"]`).
 */
private fun List<String>.toJsonArray(): String = joinToString(",", "[", "]") { "\"$it\"" }
/**
     * Serialize this map into a compact JSON object string.
     *
     * Keys are written as JSON string members and values as numbers (no additional spacing),
     * e.g. `{"key":123,"other":4}`. Note: keys are not escaped beyond being wrapped in quotes.
     *
     * @return A compact JSON object representation of the map.
     */
    private fun Map<String, Int>.toJsonObject(): String =
    entries.joinToString(",", "{", "}") { "\"${it.key}\":${it.value}" }

/**
 * Produce a project-level health report about toolchains, Kotlin targets, Compose usage and the unified OpenAPI spec.
 *
 * Collects module reports, summarizes counts by Java toolchain and Kotlin JVM target, and detects modules with
 * inconsistent Java/Kotlin targets or Android modules missing Compose. It also inspects app/api/unified-aegenesis-api.yml
 * (if present) to report operation and `operationId` counts, coverage, age (days) and staleness.
 *
 * If `jsonFlag` is true the report is printed as a single JSON object; otherwise a human-readable summary is printed.
 *
 * @param jsonFlag When true, print machine-readable JSON; when false, print a human-readable report.
 */
private fun Project.runConsciousnessHealth(jsonFlag: Boolean) {
    val reports = collectModuleReports()
    val javaGroups =
        reports.groupBy { it.javaToolchain ?: "unspecified" }.mapValues { it.value.size }
    val kotlinGroups =
        reports.groupBy { it.kotlinJvmTarget ?: "unspecified" }.mapValues { it.value.size }
    val expectedJava = "24"
...
tasks.register("consciousnessHealthCheck") {
    group = "aegenesis"
    description =
        "Full health & consistency report (toolchains, Kotlin targets, plugin presence, OpenAPI freshness). Use -Dformat=json for JSON output."
